...
namespace LibraryCatalogSystem
{
	public class Library
	{
		...
		public Dictionary<ulong, Book> LibrarySelection = new Dictionary<ulong, Book>();

		public static Dictionary<ulong, Book> ConvertCSVToDictionary(List<Dictionary<string, string>> csvData)
		{
			Dictionary<ulong, Book> libraryDictionary = new Dictionary<ulong, Book>();
			...
			foreach (var row in csvData)
			{
				...
				bool validRow = validIsbn && validIsbnNumber && validTitle && validAuthor && validStatus && validStatusEnum;

				if (validRow)
				{
					Book book = new Book(title, author, status);
					libraryDictionary[isbn] = book;
				}
				else
				{
